...
| 问题编号 | 功能影响 | 用户体验影响 | 安全/数据影响 | 运维影响 | 开发/团队影响 |
|----------|----------|--------------|---------------|----------|---------------|
| P1 | 分片数据可能损坏，合并后文件无法使用；用户上传大文件后发现损坏需重传 | 文件损坏无法提前知晓，仅在使用时发现；大文件重传耗时长，严重影响满意度 | 损坏文件占用存储空间；可能传播给其他下载用户 | - | - |
| P2 | 网络波动导致上传失败后需手动重试；不稳定网络环境下上传成功率显著降低 | 用户需持续关注上传状态，无法安心进行其他操作；频繁失败导致放弃上传 | - | 增加客服支持成本，用户投诉增多；上传成功率指标下降 | - |
| P3-P5 | PR 无法自动触发检查，代码合并风险增加；Docker 镜像未验证，部署时才发现构建问题 | - | - | - | 缺少自动化测试，回归测试依赖人工；问题发现延迟，修复成本增加；新成员提交代码缺少自动验证；代码审查缺少自动化辅助 |
| P6 | 无法支持视频预览、转码等高级功能；企业级视频管理场景受限 | - | - | - | - |
| P7 | - | - | 可能上传恶意可执行文件（如 .exe、.sh）；服务器存储空间被非预期文件占用 | 需要人工审核上传文件类型；增加服务器安全管理难度 | - |
